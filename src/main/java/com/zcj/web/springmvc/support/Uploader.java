...
public class Uploader {
    ...
    public static class Builder {
         ...
         * <p>
         *
         * @param basePath 例：Configuration.getRealPath()
         * @return
         */
         ...
         *
         * @param allowUploadPaths 相对basePath的目录
         *                         <p>
         *                         例："/upload/.*"、"/upload.*"
         * @return
         */
         ...
         * <p>
         *
         * @param servers 例："192.168.1.111:22122"
         * @return
         */
         ...
         * 如果同时设置了允许上传的后缀和不允许上传的后缀，不允许的配置会被忽略
         *
         * @param unAllowSuffixs 不允许上传的文件后缀
         *                       <p>
         *                       例：".jsp"、".exe"
         * @return
         */
         ...
         * <p>
         *
         * @param waterPath 水印图片相对basePath的路径
         *                  <p>
         ...
         * <p>
         *
         * @param width  图片宽度达到指定值才添加水印
         * @param height 图片高度达到指定值才添加水印
         * @return
         */
         ...
         * <p>
         *
         * @param width  图片以固定宽度存储
         * @param height 图片以固定高度存储
         * @return
         */
    ...
    public Object go() {
        ...
        if (uploadType == Builder.UPLOAD_TYPE_LOCATION || uploadType == Builder.UPDATE_TYPE_ALIYUN) {// 验证本地上传时的参数
            if (UtilString.isBlank(basePath)) {
                throw new BusinessException("请配置文件存储的根路径");
            }
            ...
            if (!allowUpload) {
                throw new BusinessException("文件上传失败：目录" + filePathMatch + "不允许上传文件");
            }
        }
        if (uploadType == Builder.UPLOAD_TYPE_FASTDFS) {// 验证FastDFS上传时的参数
            if (fastDFSServers == null || fastDFSServers.size() == 0) {
                throw new BusinessException("请配置FastDFS的服务器地址");
            }
            ...
            if (aliyunOssConfig == null || UtilString.isBlank(aliyunOssConfig.getAccessKeyId()) || UtilString.isBlank(aliyunOssConfig.getAccessSecret())
                ...
                throw new BusinessException("请配置阿里云OSS参数");
            }
        }
        ...
        if (!(request instanceof MultipartHttpServletRequest)) {
            throw new BusinessException("请设置正确的上传方式");
        }
        MultipartHttpServletRequest rm = (MultipartHttpServletRequest) request;
        Map<String, MultipartFile> mfs = rm.getFileMap();
        if (mfs == null || mfs.size() == 0) {
            throw new BusinessException("请确定上传的内容中包含文件域");
        }
        int fileCount = 0;
            ...
            for (Map.Entry<String, MultipartFile> entry : mfs.entrySet()) {
                ...
                if (!file.isEmpty()) {
                    ...
                    if (!validSuffix(suffix)) {
                        throw new BusinessException("文件上传失败：不允许上传" + suffix + "格式的文件");
                    }
                    try {
                        if (uploadType == Builder.UPLOAD_TYPE_LOCATION) {
                            ...
                            return new UploadSuccessResult(file.getOriginalFilename(), file
                            ...
                            return new UploadSuccessResult(file.getOriginalFilename(), file.getSize(),
